...
Substantiating a case for private paternalism is not easy. It involves comparing the causal effect of forcing a certain product characteristic versus the effect of letting people choose it. Because choice may be correlated with unobservables, identification 
is challenging in observational studies.  %This is not a straightforward task because choice will be driven by unobservable factors, and to address the question fully 
Moreover, to have a complete picture of the benefits of forcing, one would want a counterfactual for the outcome of both choosers and non-choosers. A large behavioral literature has illustrated that allowing borrowers or savers to choose commitment-style products increase savings in an Intention-to-Treat sense \citep{thaler2004save, prina2015banking, brune2016facilitating, callen2019headwaters, Pascaline, Ashraf}, but this literature has rarely inquired about the benefits of imposing commitment.\footnote{Selection on gains has such wide acceptance that economists often impose it in their models (for example, in the study of migration \cite{chiquiar2005international} and occupational choice \cite{heckman1990empirical}. In microfinance, a context close to ours, \citep{Murdoch} claims that positive selection exists, citing a positive correlation between displaying present biased preferences in surveys and selecting into microfinance as evidence of borrowers' actually demanding the discipline imposed by frequent payments.}  In this paper we illustrate how a three-armed, `controlled choice' design can be used to form counterfactuals for both choosers and non-choosers of commitment. We implement such a design in a high-stakes and policy-relevant environment, enabling us to test selection-on-gains as well as isolating the impact of paternalistic policies.
...
Against this backdrop, we implement a multi-arm randomized control trial covering close to 10,000 pawnshop clients in 6 branches of Lender $P$ in Mexico City, using a design built to test the demand for and impact of payment structure in pawn repayment contracts. Our control arm illustrates the costs of the status quo contract; fully \textcolor{blue}{56\%} of borrowers default and so lose their pawn.\footnote{These high default rates are not uncommon, in the US default is also high at 15\% (\url{https://tinyurl.com/yc2x5bjf}).} We then implement a `commitment choice’ arm, in which borrowers are able at the time of taking a loan to opt into a structured repayment contract, in which they are asked to make three monthly payments rather than one balloon payment at the end, with each monthly payment including the accrued interest at that time, and borrowers levied a nominal fee of 2\% of that month's payment in case of delinquency as a reminder/reinforcement of the importance of these interim payments. In addition, we include a `forced commitment’ arm, in which all borrowers are required to repay using the monthly payment structure. We will use the terms monthly payments, structured payments, and commitment contract interchangeably.

We call this randomization of voluntary and forced commitment a controlled choice design, and it allows us to address several key questions.  First, does using more structured repayment contracts help pawnshop borrowers to save on financing cost. Second, do borrowers recognize this benefit and demand commitment? % demanding commitment in sufficient numbers?  
Finally, and most uniquely, because we incorporate the forced commitment arm, we are able to ask whether the \textit{right} borrowers voluntarily demand commitment, gaining point identification for the Treatment on the Treated (ToT), the Treatment on the Untreated (TuT), and the average selection on gains. 
	
Our results suggest that the structure of the commitment is strongly effective in preventing default, with the average individual in the forced arm paying financing costs inclusive of fees that are \hl{20\%} lower than the control, while default decreases by 6.5 percentage points (15\% of the mean). In terms of Annual Percentage Rates, the financial cost of borrowing is reduced by 34 percentage points. That is, imposing a commitment structure it is possible to save borrowers money by charging them fees. These results are robust qualitatively to including transport costs of going to the branch plus losing a day's wage for each visit, using the subjective instead of the gold value of the pawn, and imputing some adjustment for liquidity lost by requiring monthly payments. 

The monthly payment contract seems to be achieving these cost savings by speeding up payments and by generating an early bifurcation of borrowers into those that will recover the pawn and those that will not, saving the later payments towards recovery (which are lost in the case of default). In the treatment group, the first payment occurs 13 days earlier, and the fraction recovering on the first visit by is 7.9 percentage points higher. The commitment contract also decreases fraction of borrowers making a payment and not recovering by 7 percentage points. Treatment effects are concentrated in the intensive margin as treatment does not affect the fraction of clients who pay a positive amount towards pawn recovery. Instead, the commitment contract induces people who would otherwise pay something towards recovery to pay more and faster, as well as inducing others who would have made partial payments and anyways defaulted to instead pay nothing.  Both groups save money.

In spite of a large average treatment effect documenting financial cost savings, given the opportunity in the choice arm, only 11\% of borrowers actually choose payment structure. If the effect of payment structure were homogeneous, this would be enough to conclude that the 89\% who did not choose it would have been financially better off if they had. However, we test and reject the null hypothesis of homogeneous treatment effects using the method of \cite{chernozhukov2018generic}. Can the borrowers who did not choose commitment be those who simply don’t need it? We address this question by sequentially imposing more structure in the problem. 

We start by not imposing assumptions and bounding the distribution of individual treatment effects as in \cite{fan2010sharp}. Using bounds we find that at least 30\% of individual borrowers benefit from commitment, implying many borrowers did not demand the structured payment contract even though the treatment effect would be positive for them. Then, we impose the twin exclusion restrictions necessary for estimation of the ToT and TuT ---that the effect of choosing or not choosing is the same as being assigned or not assigned the contract, i.e. that the act of choosing itself is irrelevant.\footnote{This is a frequently used assumption even if often implicit. For instance, a significant literature uses schooling compulsory laws to estimate the return of one more year of school, and then often interprets that schooling, in general, has this return.}  With these assumptions, both treatment effects are point identified, as is the average selection on gains, the average selection on levels, and the average selection bias.  Our results indicate that while selection on gains are present in our sample, the financial cost savings of the TuT is still large (at \$356 pesos, it is equivalent to 24 percentage point savings in APR). Hence, on average borrowers that did \textit{not} choose the frequent payment contract would have benefited from having that contract.\footnote{While our estimated ToT is roughly twice the TuT, it is imprecisely estimated due to the low compliance rate, and the hypothesis of ToT=TuT is rejected only with 13 percent confidence for the APR and 12 percent confidence for default.}  We also estimate that those that would not choose if given the opportunity would have lower status-quo outcomes (i.e. the average selection bias is negative). Finally, assuming the exclusion restriction for subsets of observable characteristics splits generated by a causal forest allows us to establish that more than \hl{70\%} of would-be non-choosers would save on financial cost by being assigned to frequent payments. That is, the benefits of imposing a monthly payment are distributed broadly. \hl{TuT benefits are particularly large for XXX CHARACTERISTICS, and ToT benefits for those with XXX.} 
...
What explains the persistence of a contract so contrary to borrowers’ interests?  First, we investigate whether simple impatience could explain the dissonance (our formula for financial does not use discounting, and the contract features near-term costs in return for longer-term benefits).  A simple simulation exercise suggests that at discount rates up to 4,000 percent per annum our core impacts on financial cost remains significantly negative, suggesting that discounting is unlikely to explain our results.  Next, we look at time inconsistent preferences, the workhorse behavioral explanation for the benefits of commitment.  In our context, not only is time inconsistency only a weak driver of selection into commitment, but the TuT of forcing commitment arises almost entirely among those who are \textit{not} time inconsistent.\footnote{\textcolor{blue}{Although telling, we acknowledge that our measure of present bias is simple (as its survey-based and the survey was necessarily short). For this reason this result needs to be interpreted with care.}}  In this pawnbroking sample we document substantial levels of over-optimism among borrowers, and it has predictive power for our results. %which we believe explains our results. 
Not only do 74\% of borrowers say that they will recover their pawn with perfect certainty (whereas only 43\% actually do), but the impact of forcing on those who would not choose commitment is almost perfectly confined to the sub-sample who reported sure confidence in recovery. Hence, in this context it appears that the explanation for low demand for commitment among those who benefit arises from an incorrect belief that one has no need to work to avoid default.  

%Those most likely to benefit from the commitment contract and \textit{not} choose it are the individuals who most systematically over-estimate their own probability of repayment at the time of taking the loan.\footnote{More generally \hl{demand for commitment is higher among those with XXX characteristics.}}  Second, the existence of commitment-style repayment contracts in pawnshop lending is novel in Mexico. The dramatic improvements in borrower income arising from the commitment contract come directly from the pockets of lenders, reinforcing that the pawn industry as a whole has a strong interest in retaining the status quo.

%do we want this paragraph on learning?
Does learning eventually overcome the low demand for commitment? For learning to happen clients would need to experiment the frequent payment contract, and this was not the typical contract in the industry. But even if the contract was experienced, borrowers may fail to learn or learn only slowly. We find that those who are forced to the monthly payment product are 6.7 percentage points more likely to become repeat customers than those assigned to the status-quo contract. One way to interpret this is \textcolor{blue}{as `learning they like pawn-borrowing', i.e. having experienced the frequent payment contract (and the lower financial cost of borrowing that come with it), they like pawning and pawn more.}\footnote{A second intepretation is that the iliquidity created by the monthly payment contract makes them want obtain second loan to pay the first one. This second explanation is unlikely since borrowers obtain a second loan not within the first 90 days of the first loan when payments are due; moreover, the correlation is also present conditional on having paid down the first loan. Borrowers tend to pledge a different piece for the second loan, so that recovery of the first pawn is also an unlikely explanation. \textcolor{blue}{A different test, about `learning they like monthly-payments'} is whether conditional on coming again, clients would choose frequent payments. We test this and we cannot reject that demand is \textit{not} higher after experiencing the monthly payment contract, \textcolor{blue}{consistent with no learning or slow learning that the monthly payment contract is better than the status quo contract}, however sample sizes are small and \hl{the minimum detectable effect is XXX.}} %\textcolor{blue}{making this more rigorous test of learning inconclusive and left for future research. Having said this, after more than 100 years in operation, the biggest pawn lender in Mexico and most others still do not offer structured payment contracts.}
...
%Replace this paragraph with discussion of optimal targeting of paternalism
Our results suggest that pawn lending markets capture borrower wealth in ways not well summarized by interest rates. By over-collateralizing and then structuring loan contracts in a manner encouraging of default, lenders are able to extract substantial value independent of the interest charged.  Further, the `nudge’ approach generally favored by the behavioral literature (voluntary commitment) may not be adequate. We hesitate to give precise policy recommendations since we don't have good measures of welfare. Even though the monthly payment contract dramatically reduces financial cost, it could potentially generate lower (or higher) anxiety. Moreover, frequent payment requirements may decrease welfare of highly impatient borrowers.\footnote{The paper presents an back of the envelope exercise that suggests annual discount rates of 4000 are necessary to nullify the reductions in financial cost we estimate. \hl{Note also that since the fee for skipping payment is 2\% of the payment due, the illiquidity imposed by the contract is bounded.}} %Second, given the non-standard nature of the finding, it is likely that we would have to adopt and estimate a particular behavioral model of demand for commitment, which we don't need to do to make the point that TuT is positive.
...
Our paper speaks to several strands of the literature.   First, we contribute methodologically to the large literature on treatment effect heterogeneity by showing how an experimental design with two forcing arms and one choice arm allows for identification of the ToT, the TuT and other important parameters under minimal assumptions.  A large literature inspired by \cite{heckman1979sample} and \cite{angrist1996identification} has used an exclusion restriction to instrument for the LATE; rarer are studies that have compared forcing to choice to instrument for the TUT (such as looking at compulsory high school requirements \citep{oreopoulos2006estimating}, and one at technology that defaults individuals into variable pricing of electricity \citep{fowlie2021default}.  When the three arms (control, choice, and forcing) are randomized in the same environment we gain a clear and policy-relevant window into how selection into treatment relates to heterogeneity in treatment effects.  This comparison of treatment effects across the compliance distribution is closely related to the evaluation of Marginal Treatment Effects, and hence speaks to the large observational literature using parametric assumptions and continuous instruments to estimate MTEs , such as \cite{heckman2007econometric} and \cite{cornelissen2018benefits}, and \cite{Walters}.


%\cite{aronow2013beyond} and \cite{angrist2013extrapolate} assume no selection on gains to estimate \hl{XXX}. \cite{Walters} achieve identification of the TuT by using parametric model and a distance instrument, which requires the model to be correct and the instrument 



Second, we join a more nacent literature shedding light into private paternalism. In the context of food choice, \cite{Sprenger} elicit dynamic inconsistencies, and then find that demand for commitment is negatively correlated with such inconsistency. In the context of school choice, \cite{Walters} finds that students that select into more effective schools have smaller treatment effects from attending than those that select out. Besides being very different contexts from ours, we rely on very different methods. In contrast to \cite{Sprenger} we will directly point identify the TuT, obviating the need for elicitation of preferences to test for negative selection. In contrast to \cite{Walters} we use randomization while he needs a structural model and distance to school as an instrument. \todo[inline]{FRANK, can we say something more precise than ``a structural model''?}


Finally, our results speak to the financial literature that examines the effect of contract structure on repayment.  While research in microfinance markets has not found the same benefits from providing a more regularized repayment environment \citep{Pande}, those experiments are performed on top of highly structured contracts and among borrower pools who may have selected into that type of lending precisely because it provides structure. This may explain why there is almost no default in the control group in \citep{Pande}. Second, we broaden the analysis of the extent to which voluntary commitment mechanisms are taken up and effective.  Like our study, commitment demand has been found to be low in a number of contexts (\cite{Ashraf}, \cite{Gine}, \cite{Ted}, \cite{Royer}, \cite{Sprenger}). Other papers have found more robust demand for commitment (\cite{Kremer},  \cite{Casaburi}, \cite{Alcohol}, \cite{AprajitP&P}, \cite{Pascaline}). But unlike all these papers we can point identify the benefit the non-takers would have had if they had taken up and the benefit for the takers, and thus can talk more rigorously about private paternalism in this context.
...
%In contrast with \cite{John} we don't let the clients choose the size of the fee. \cite{John} shows that doing this results in costly choices by (partially) naive present biased consumers. Our fee is much smaller than the median in her experiment. 
...
A natural starting point for understanding the role of voluntary commitment is the determinants of choice.  Figure \ref{determinants_choose} provides coefficient plots for univariate and multivariate Linear Probability Models explaining takeup of commitment in the Choice arm.  Contrary to the standard behavioral motivations, none of our measures of self control predict takeup, neither the metric arising from the paired discounting questions (e.g. those who are more impatient over immediate rather than future delays), nor a simple survey question asking whether they are tempted to mis-use money in their financial lives.  Individuals who report having trouble paying bills are actually \textit{less} likely to take commitment, as are those who have pawned before.  The only determinant in this figure that represents something like sophistication is that those with more education are more likely, in both univariate and multivariate regressions, to choose commitment.  Overall, we observe few strong determinants of commitment choice and the story does not appear to be a behavioral one.
...
In a world of heterogeneous treatment effects, however, low demand for commitment could still be consistent with \textcolor{blue}{standard rational borrowers.}
...
In this section we present a number of econometric exercises that shed light on this question, leveraging unique features of our experimental design. 
...
\todo[inline]{@Isaac: Need a sentence saying what the results are: at least such and such percent benefit from forced commitment}
...
Our assumption-free bounds from the preceding section show that more than 11\% of borrowers would benefit from a policy of forced commitment. 
...
For details, see Appendix \ref{append:exclusion}.
...
\section{Examining paternalism}
\label{exploring}

\subsection{Why is paternalism working in this context?}
\label{why_paternalism}

The behavioral literature has highlighted voluntary commitment as an attractive way of allowing the `right' people to self-select.  A discussion of compulsory commitment is necessarily paternalistic, and by its nature focuses on impacts among those who would not have selected commitment voluntarily.  From the lens of rational choice it is unsurprising that we find the $\text{ToT}>\text{TuT}$, but the argument for compulsory treatment centers on the more surprising result that we find $\text{TuT}>0$.  We now investigate several potential explanations for why this might be the case in our context.  

\paragraph{Learning.} A first explanation involves learning.  We were bringing a new contract into an environment that had not previously featured commitment; perhaps clients required experience to understand its benefits?  Given the strongly positive impact of forcing, our clearest hypothesis would be that clients forced once would choose commitment if subsequently given the chance to do so.  While the sample size for this is small, we do have a group of 240 clients who were exposed to the experiment in one round and then returned to take another loan at a branch where the choice of commitment was offered.  Table \ref{learning_table} shows the results of this analysis.  While this regression is underpowered, the point estimate on prior assignment to the commitment contract is actually negative. Hence the window available to us on learning provides little comfort that the lack of voluntary compliance is simply a matter of inexperience with commitment.\footnote{\textcolor{blue}{Risk could be another alternative potential explanation for low take up of the commitment contract. Appendix \ref{App_FOSD} shows however that the distribution of financial cost savings of the monthly payment contract first-order stochastically dominates that of the status quo contract, so that (assuming these are the distributions faced by borrowers) the former should be preferred by borrowers regardless of their degree of risk aversion. See the Appendix for more details.}}


\paragraph{Time discounting.} Discounting is a second explanation for our results.  Using no discounting we find a decrease in the financial cost of credit, but the intervention inherently imposes up-front costs (early payment) in return for delayed benefits (recovery of pawn).  Highly impatient individuals might therefore rationally choose not comply although our measure would show a benefit for them in undiscounted returns.  To investigate this we can calculate the Net Present Value of the $TUT$ on financial cost given the %average
timing of repayment and pawn recovery, for different discount rates. Figure \ref{fc_discount_rates} shows the results of this exercise, illustrating that non-choosers continue to see significant decreases in NPV financial costs up to annual discount rates of 4,000\%, and the NPV remains negative at 10,000\% discount rates.  Given this, we think that discounting is unlikely to explain non-compliance among those who appear to benefit. 


\paragraph{Present bias.} If commitment benefits among non-compliers cannot be explained by rational choice, the canonical behavioral story would center on time inconsistency.  While commitment is useful to anyone with hyperbolic time preferences, only those who are sophisticated will demand it.  A large share of naive hyperbolics in the population could therefore drive a large $TuT$.  Our baseline survey included standard questions about discount rates between today and a month in the future versus discount rates between three and four months in the future.\footnote{The remaining exercises in the paper will use information from the intake survey, and hence use only the subset of individuals who responded to the survey.  While this is a selected sample, we believe this to be benign in that the impacts estimated using the full and survey-respondent sample are very similar.}  This allows us to classify borrowers who display more impatience over immediate delays as time inconsistent.  This measure of financial hyperbolicity is widely used in survey research, although it is not without problems.\footnote{Our measure is dichotomous, and is not incentivized.  Recent empirical work has shown the superiority of more sophisticated measures such as Convex Time Budgets \citep{andreoni2015measuring} as well as questioning the interpretation of measures of hyperbolicity not based on consumption \citep{andreoni2012estimating, cohen2020measuring}, suggesting that real effort tasks provide a better measure \citep{augenblick2015working}.  Given that we had only a few minutes to interview real pawnshop clients prior to a commercial transaction, our simple measure was a necessary compromise.}   
...
We can calculate the relative contributions of any two groups $X_i=0$ or $X_i=1$ to the $TuT$ among the non-compliers as follows:
...
\paragraph{Sure confidence.} A second behavioral explanation may arise from the fact that  74\% of the control participants believe they have a 100\% chance of recovering their pawn, while in truth only \hl{56\%} will do so.  This suggests a market characterized by over-optimism.  Incorrect expectations about recovery probabilities can explain low take-up in that individuals who believe they will repay are rationally uninterested in incurring costs to induce repayment.  Hence over-optimistic expectations in recovery probabilities will translate directly into under-adoption of preventive measures.   Similar to the definition of hyperbolicity above, we define a binary attribute we call `sure-confidence' which is individuals who say at the time of borrowing that they have a 100\% probability of recovering the loan.  

\todo[inline]{check in all the text we have used 56\% as the number that dont recover.}


Figure \ref{tut_beh_partition} shows the relative contributions of present bias and sure-confidence for the $TuT$.  In each figure the orange bar represents the contribution from those who do not possess the attribute, and the blue bar from those that do.  We see that the forcing effect is localized mostly to those who are \textit{not} time inconsistent, contradicting naive hyperbolicity as the justification for paternalism.  Instead, we see sure-confidence being a category that completely isolates the $TuT$, suggesting that forcing works for those who do not consider repayment structure because they don't think they need it.  Our measure of hyperbolicity is based on unincentivized responses in a short survey and so is likely to be noisy; nonetheless we see no evidence here that it drives the forcing effect.  Rather, this analysis suggests that paternalism is effective in this case is because of a group of overconfident borrowers who, heedless of the possibility of default, do not choose commitment and yet would benefit substantially from receiving it.


If the attribute of `sure confidence' proves so important in explaining the TUT, what are the determinants of this characteristic?  In Figure \ref{determinants_sure} we plot the coefficient estimates from a regression that explains sure confidence with a battery of individual-level characteristics.  Older males are more likely to be sure confident, as are those with more education.  Taken at face value, it also appears to be the case that it is those with less financial stress, those who have had less trouble paying bills, and those more relied upon financially by family members.  However, through a behavioral lens it is also possible that the type of person who is over-confident in their ability to repay a loan is also maintaining financial fictions in other domains of life, providing answers to the baseline survey questions that exaggerate their degree of economic security.  In any case it appears that this attribute may be difficult to target with objective demographic criteria, a point to which we return.


\subsection{Analyzing Choice versus Paternalism Using a Causal Forest} \label{sec:RF}


\paragraph{Causal forests.} Figure \ref{fig:FanPark} uses a completely non-parametric approach to understand winners and losers from paternalism at the individual level, but these bounds are relatively wide.  To make more precise statements, we can invoke a selection on observables assumption and use the Random Forest approach of \cite{athey2019generalized} to estimate Conditional Average Treatment Effects (causal RF), or the Conditional TuT and ToT (instrumental RF).  These impacts allow us to think about whether individuals made `mistakes' in their choices.  Specifically, for each client in the choice arm we want to know how much cost savings would she have gained had she chosen the opposite contract. We do this in two steps.  First, we use  \cite{atheygrf}'s generalized random forests methodology to estimate the impact of the forced commitment arm relative to the control (forced status quo) at the individual level.  The method generates a partition of the space of characteristics $X$, where each element of the partition is a leaf, and each leaf contains treatment and control clients with similar characteristics $x \in X$ which are used to calculate unbiased treatment effects for that leaf. The optimal partition is that which minimizes expected mean squared error of treatment effects predictions out-of-sample.  The tree is honest because it uses one sample to build the tree and another one to estimate treatment effects within that tree.  \cite{atheygrf} show that their treatment effects are asymptotically normal, which allows us to construct confidence intervals for our treatment estimate in each leaf. This method gives us the causal effect on financing cost of getting the forced commitment contract instead of the status quo contract for a client $i$ with characteristics $x_i$; call it $TE^{F}_{i}(x_i)$.\footnote{Figure \ref{causal_tree1} and \ref{causal_tree2} provide an illustration of the splits (leaves) that emerge from our implementation of the RF. \todo[inline]{I think we should list in a footnote which are the `wide' variables we are using.}}  
...
%Figure \ref{choose_wrong}(a) plots on the Y axis the fraction of clients that made a choice ``mistake'' and on the X axis the threshold $z$. When $z=0$, we calculate that about 82\% of clients in the Choice arm chose contracts that induced \textit{higher} financial cost. Most mistakes are failures to choose the fee-commitment contract ({97}\%), while only {4}\% of those who chose the fee-commitment contract would have been better in the status-quo one. The average mistake in pesos is \$250. %(90\% confidence interval [140,390]). 
 ...
 Figure \ref{choose_wrong}, Panel A shows the results of this exercise, reinforcing the general superiority of commitment.  Among choosers, only 25\% would have had superior outcomes had they not complied, whereas among non-choosers almost 80\% made a mistake according to this definition.  Because this arm is comprised of 89\% non-choosers, the overall rate of mistake in the arm was around 70\%.  As we increase the threshold on the size of the mistake necessary to be classified as having made the wrong choice, by definition the share in both groups falls, and among choosers the number making an error falls quite quickly to zero.  
...
Panel C of this figure replicates this exercise for the forced commitment arm, with the difference being that the relevant treatment effect to match is now the ATE of the forced commitment arm.  This allows us to identify the winners and losers from paternalism in a nuanced manner, since members of groups with CATEs$<$0 are hurt and everyone else is helped.  Because the ATE$>$TUT, this exercise results in an even larger fraction of individuals benefiting from commitment; here we find that fully 87\% of individuals are better off being forced into commitment than staying in the control. This now allows us to make a stronger statement in favor of paternalism; not only is the case that forcing commitment generates large benefits on average, but it is also generating benefits for almost 90\% of the forced population.



\subsection{Can we target paternalism?}

Despite having shown such a large majority of individuals benefit from paternalism, it is still natural to question whether we are able to target commitment in such a manner as to only force it upon those who benefit.  For a financial firm to engage in this type of targeting under real-world circumstances, we must impose several constraints on the targeting rule.  First, it will in general not be possible to use the numerous subjective and unverifiable questions we asked in the baseline survey as inputs to the rule because the answers to these questions can be manipulated by the clients, and would likely change when they become incentivized.  Secondly, it will not be attractive for a commercial firm to ask individuals whether they want to voluntarily accept commitment and then to force upon those who say no against their will. So the choice variable itself cannot be used to target other than in a voluntary program. This leaves us with only a few objective covariates that could be used to target: age, gender, HS education or above, and whether that individual has ever pawned before.  

\paragraph{Narrow inputs targeting.} To  consider targeting effectiveness we must begin from an individual-specific ground truth, which we take from our estimated RF from Section \ref{sec:RF}  which used the `wide' set of covariates (wide RF). We then analyze how well we can predict the losers from paternalism using the `narrow' set of non-manipulable covariates listed above (narrow RF). 
Figure \ref{wide_narrow_forests} shows the relationship between the CATEs estimated from the RF using the `wide' and `narrow' covariates.  Not only is it visually obvious from this figure that we generate substantially less heterogeneity when using the narrow covariate set, but in fact the Chernozhukov test for heterogeneity on the narrow RF is no longer significant.  This constitutes preliminary evidence that the commercially feasible covariate set we collected will not be effective at targeting paternalism.  

\paragraph{Targeting assessment.} To more directly examine the implications of attempting to target commitment, we use two different techniques to predict who should be assigned to the commitment arm.  The first of these is the narrow RF displayed above paired with the decision rule which assigns clients to forcing commitment in a  leaf if and only if  the CATE of the Forcing arm is positive. In the second we estimate a standard Logit model using as outcome an indicato of positive benefit from forcing commitment as estimated in the wide RF, and the narrow targeting variables as the explanatory variables.  Figure \ref{targeting_rules} plots the same densities of net benefit as were shown before, where we now take the wide RF as the ground truth of the benefits of forcing everyone into commitment, and the welfare under the two targeting rules shown for comparison.  Given the poor explanatory power of the narrow criteria it is unsurprising to see that the information gain from using these variables is small; the fraction of the whole sample benefiting when we go from universal forcing to narrow RF targeted forcing rises only marginally from 86.7\% to 88.4\%.  When we look at the Logit targeting we see that it has actually made targeting substantially worse; this behavior is driven by the confluence of the poor explanatory power of the narrow variables with the fact that the Logit matches the correct sample fraction of ones, therefore making numerous Type II errors in an environment where commitment is beneficial for most. %and hence actually reduces the share benefiting.  

%The comparative points are illustrated in
Table \ref{hit_miss_rule} shows Type I and Type II error rates for each of six rules:  all to control, all to treatment (Forcing), optimal, Narrow RF targeting, Logit targeting, and Choice.  While the Narrow RF is able to correctly assign roughly half of those who do not benefit from commitment to control, it also incorrectly allocates 5.4\% of the sample that would have gained from treatment to control, and so on net it only improves the correct targeting rate by about 1.5 percentage points relative to universal Forcing.  The Logit is less successful at predicting benefits and in particular makes many Type II errors, resulting in a net targeting performance that is actually worse than universal forcing. The underlying cause of the weak performance of targeting is that the attribute that predicts loss from forcing (not being sure confident and not choosing the commitment) is largely behavioral and not easily predicted with basic covariates.  Self-targeting through choice proves to be little better than assigning everyone to the control, given the low take-up rate and the presence of both Type I and Type II errors in the Choice arm.  Over 71\% of individuals in the Choice arm made a mistake, weighting the results among choosers and non-choosers by their frequency. The takeaway is that given low take-up, the large fraction of the sample benefiting from commitment, and the weak predictive power of the narrow covariates, in this case universal paternalism appears to be an attractive contract. 
...
We examine treatment heterogeneity using a range of methods, from bounds that make no assumptions, exploiting a mild exclusion restrictions and the innovative controlled choice design to gain point identificatoin of TuT and ToT, and invoking seletion on observables to estimate treatment effects at the person level. We find that a large majority of clients benefit from the commitment contract, but that when given choice between the commitment and the status quo contract only 10\% choose the former one and thus forgo important financial benefits. We also find that overconfident clients experience larger treatment effect cost savings from the commitment contract but at the same time have smaller demand for it. (\hl{Is it true that they have smaller demand? Which Figure shows this? If so we should say this above and not only in the conclusion})
...
%\textit{Do file: } \texttt{determinants_choice_sure_confidence.do}       
\end{figure}
...
\begin{figure}[H]
    \caption{Wide and narrow forests}
    ...
    \end{subfigure}
    \end{center}
      \scriptsize  This figure plots the relationship between the Athey-Wager heterogeneous treatment effects estimated using the `wide' set of determinants (all available covariates), versus the `narrow' set (only age, gender, HS education, and previous borrowing).  The scatterplot graphs one estimate versus the other, and the histograms show the densities of the respective heterogeneity estimates.
