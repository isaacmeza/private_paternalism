...
\author{Craig McIntosh \and Isaac Meza \and Joyce Sadka \and Enrique Seira \and Frank DiTraglia   \thanks{Seira: ITAM/MSU, enrique.seira@gmail.com (corresponding author); McIntosh:  University of California San Diego, ctmcintosh@ucsd.edu; Meza: University of California at Berkeley, isaac.meza@berkeley.edu; Sadka: ITAM, jsadka@itam.mx; DiTraglia: Oxford, frank@ditraglia.com} }
...
\label{eq:exclusion}
\end{equation}
\ref{eq:exclusion} says that each participant's potential outcome is the same regardless of whether she \emph{chooses} a particular treatment $D=d$ or is \emph{assigned} the same treatment. Viewing $Z_i$ as an instrumental variable (IV) for the treatment $D_i$, this nothing more than the usual IV exclusion restriction.\footnote{\cite{chamberlain2011bayesian} uses an identical assumption to develop a theory of optimal treatment choice for an individual who has access to data from an RCT.} \ref{eq:exclusion} is plausible in our empirical setting and has testable implications that we fail to reject: see Appendix \todo{Fill in}{???} for details. Under \ref{eq:exclusion}, each participant's observed outcome $Y_i$ is related to her potential outcomes $(Y_{i0}, Y_{i1})$ according to 
...
\label{eq:chernreg}
\end{equation}
where $\psi_i \equiv \psi(X_i|\text{Training})$ and $\tau_i \equiv \tau(X_i|\text{Training})$.\footnote{This is a slightly simpler regression than the one proposed in equation (3.1) of \cite{chernozhukov2018generic}, which involves propensity score weights. Because the random assignment of $Z$ in our experiment does \emph{not} condition on $X$, the propensity score weights in our case are constant over $X$ and hence drop out.} As shown by \cite{chernozhukov2018generic}, the coefficients $\beta_1$ and $\beta_2$ from \ref{eq:chernreg} identify the \emph{best linear predictor} of the conditional ATE based on $\tau(\cdot|\text{Training})$, namely
\[
\beta_1 = \mathbbm{E}[\text{ATE}(X_i)] = \text{ATE}, \quad
\beta_2 = \frac{\text{Cov}[\text{ATE}(X_i), \tau_i]}{\text{Var}(\tau_i)}.
\]
If treatment effects are homogeneous we must have $\beta_2 = 0$. Rejecting this hypothesis establishes that $\tau_i$ predicts $\text{ATE}(X_i)$ and hence that $\Delta_i$ varies. Since $\tau_i$ and $\psi_i$ do not depend on the test set, inference for the regression in \ref{eq:chernreg} is straightforward conditional on the Training/Test split.  
...
\label{eq:ToT}
\end{equation}
since $\mathbbm{E}(D_i|Z_i=0)=0$ by \ref{eq:potentialTreatments}. 
...
\label{eq:TuT}
\end{equation}
since $\mathbbm{E}(D_i|Z_i = 1)= 1$ by \ref{eq:potentialTreatments}. 

\ref{eq:ToT} and \ref{eq:TuT} are useful for understanding why our design identifies the ToT and TuT, but they are less convenient for estimation and inference. In Appendix \todo{Frank: fill in}{???}, we show that
\begin{align}
\label{eq:ToTreg}
...
\]
The point estimate of this difference can be constructed directly from the results of the IV regressions described above. Because the respective estimates appear in different equations, however, a bit more work is required to obtain a standard error for $(\text{ToT} - \text{TuT})$. The details are straightforward but somewhat tedious, so we present them in Appendix \todo{!!!}{???}. A companion STATA package accompanying this paper provides cluster robust inference for the average selection on gains, along with two related quantities: the average selection bias (ASB) and average selection on levels (ASL)
\[
\text{ASB} \equiv \mathbbm{E}[Y_{i0}|C_i = 1] - \mathbbm{E}[Y_{i0}|C_i = 0], \quad 
\text{ASL} \equiv \mathbbm{E}[Y_{i1}|C_i = 1] - \mathbbm{E}[Y_{i1}|C_i = 0].
\]
We show that these latter two quantities are point identified in Appendix \todo{!!!}{???}.
...
%\end{bmatrix}
%\]
%where $\boldsymbol{\Sigma}$ is a covariance matrix that can be estimated from the residuals of \ref{eq:ToTreg} and \ref{eq:TuTreg}. Under iid observations, 
%
...
%\textit{Do file: } \texttt{soft\_te,do, fc\_robustness\_soft.do}    
\end{figure}
...
\begin{figure}[H]
    ...
    \end{subfigure}
     \begin{subfigure}{0.475\textwidth}
        \caption{Default}
        \centering
        \includegraphics[width=\textwidth]{Figuras/benefit_choice_tau_def.pdf}
    \end{subfigure} 
    \end{center}
