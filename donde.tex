...
\noindent where $t$ indexes days, and $\mathds{1}(\text{Default}_i)$ is an indicator function for defaulting. Even though we observe transactions at the day level, \textcolor{red}{we don't use discounting} since the period of loan is only 90 days. \textcolor{red}{In robustness checks we do show that results are virtually unchanged when applying a wide range of time discounting factors.}  
...
As a second measure of cost we calculate the Annual Percentage Rate (APR) in order to express the cost as a percentage of the loan, per year.\footnote{Loan term takes into account all the loan period, including those extended through refinancing}. The standard definition is given in the following formula:
...
Two more subtle form of sample selection arise if the treatments induce borrowers to repawn in different ways (which they do), or to pay off loans at a different rate (which they do as well).  We pursue a simple approach to these problems, by using only the first experimental pawn taken by a given borrower during the study window (ignoring all subsequent loans), and by defining outcomes in a manner that is consistent whether or not the loan had been closed at the time that the study observation window ends (e.g., `loan was repaid', which is defined even if the loan had not been completed, versus `loan defaulted', which is not).  We return in the subsequent section to more detailed analysis of the implications of these two issues. 
...
As we explained above, loans can be extended for an additional 3 months by paying the interest owed and restarting the loan under the same treatment conditions. This means that some loans extend for more than 3 months. We consider the cost 
...
In Appendix \hl{???} we show how to use the residuals from \eqref{eq:TuTreg} and \eqref{eq:ToTreg} to carry out cluster-robust inference for $\text{ASG}$. 
Our companion STATA package automates this procedure.
As mentioned above, the randomized choice design also identifies the average selection bias (ASB) and average selection on levels (ASL).
In particular,
\begin{align}
\label{eq:ASB}
...
\label{eq:ASB1}
    \mathbbm{1}(Z_i\neq 1) (1 - D_i) Y_i &= \mathbbm{E}(Y_{i0}) \mathbbm{1}(Z_i = 0) + \mathbbm{E}(Y_{i0}|C_i=1)(-D_i)\mathbbm{1}(Z_i = 2) + U_{i1}\\
\label{eq:ASL0}
    \mathbbm{1}(Z_i\neq 0) D_i Y_i &= \mathbbm{E}(Y_{i1}) \mathbbm{1}(Z_i = 1) + \mathbbm{E}(Y_{i1}|C_i=0)(D_i - 1)\mathbbm{1}(Z_i = 2) + V_{i0}\\
\label{eq:ASL1}
...
For details, see Appendix \hl{???}.

\todo[inline]{Perhaps mention some other settings in which a design like this might be helpful? Need to hink about this.}
...
\todo[inline]{This next paragraph needs to be removed and/or re-written. Perhaps we want to break to a new subsection before discussing these results?}
...
\scriptsize {
\noindent This table explores treatment effects in ``intermediate variables''. Each column represents a different dependent variable following the main specification  (\ref{basic_reg}). The last row shows the mean of the dependent variable for the control group. Dependent variables are as follows: Column (1) shows the effect on the percentage of payments during the first visit. Column (2) records the probability of recovery in the first visit. Column (3) is the overall percentage of payment as fraction of the loan. Column (4) is the probability of making a positive payment and defaulting. Column (5) is the percentage of payments conditional on defaulting. Column (6) is the probability of ``selling'' the pawn, defined as not making any payment towards recovery of the item. Column (7) reports the probability of selling the pawn, conditional on defaulting. The bottom panel focuses on time related variables: Column (8) indicates the elapsed days toward the first payment. Column (9), (10), and (11) are related to the number of visits made to make payments, and columns (12), (13) speak on the loan duration.



In sum, we find that the Forced commitment contract clients pays 13.8 days earlier in average, during this first visit, clients pay 7.9\% more of the loan amount, with a 8\% increase probability of recovery. Moreover, the forced commitment contract reduces the probability of defaulting and making a positive payment by 7\%, thus another channel for savings is that treatment helps avoid making payments toward recovery but defaulting. 
However, we see no effect in the number of visits and the probability of selling the pawn.}
