...
\begin{align*}
    Financial \; Cost_i =& \underbrace{\sum_t P^c_{it}}_{\text{Discounted payments to capital}} + \underbrace{\sum_t P^f_{it}}_{\text{Payed fees}}  +\underbrace{\sum_t P^i_{it}}_{\text{Payed interests}} \\
    &\quad\qquad + \underbrace{Pawn \: Value_i - \text{I}(Recovered \: Pawn_i) \times Pawn \: Value_i}_{\text{Cost of losing pawn}}
\end{align*}
...
%First, we cannot reject that the number of pawns per branch-day is the same across arms (Panel A). Panel C shows that about 98\% of people who answered our baseline survey also pawned, and that this ratio is balanced across arms. Second, not only are the number and shares of pawns the same across arms, but also the characteristics of clients and their pawned pieces are statistically the same across arms. Panel A of Table \ref{SS} tests this with administrative data, while 
...
We now present the core experimental results, illustrating how the Commitment Choice and Forced Commitment arms differ from the control.  This comparison is particularly illuminating on the role of self-selection in Commitment, because if this self-selection mechanism works perfectly (as it would in a Roy model with all hyperbolics sophisticated) then the Forced and Choice arms will have an identical effect.  Deviations between the impacts of these two arms therefore are implicitly a test of the optimatlity of the choices made in the choice arm.  
...
\textcolor{red}{Beginning of discussion on Quantile Effects:.}
...
\begin{figure}[H]
     \caption{Several definitions of FC}
    \label{robustness_fc}
    \begin{center}
    \begin{subfigure}{0.75\textwidth}
        \centering
    ...
    \end{center}
    \scriptsize
    This figure shows treatment effects using several measures of cost. The first definition is our main definition of cost, which uses the appraised value of the pawn. The second definition uses the subjective (self-reported) value for the pawn. The third and fourth definition adds the travel cost for the treatment arms. We also compute APR versions of the above measures. 
    Finally, `APR - interests' is a version of the APR that doesn't include the interest that is mechanically saved by paying early since this is the piece of the commitment-fee contract that has a foregone liquidity cost for the borrower. The treatment effects are indicated in standard deviations, together with 95\% confidence intervals.
\end{figure}
...
\begin{figure}[H]
    \caption{Detailed comparison of Forced and Choice Commitment arms}
    \label{fc_pro4}
    \begin{center}
    \begin{subfigure}{0.45\textwidth}
        \caption{Financial cost}
...
\begin{figure}[H]
    ...
    \end{center}
         \scriptsize
     This figure plots the effects on different quantiles (15th, 25th, 50th, 75th, 85th percentile) of the distribution of financial cost, and APR, for the Commitment-fee and Commitment-choice arms.
