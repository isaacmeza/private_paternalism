...
qui putexcel B2=matrix(r(matrix)) using "$directorio\Tables\SS_2.xlsx", sheet("SS_admin") modify
...
foreach var of varlist prestamo monday num_empenio {
	...
	local p_val = `r(p)'
	qui putexcel Q`i'=matrix(`p_val') using "$directorio\Tables\SS_2.xlsx", sheet("SS_admin") modify
	local i = `i'+2
	}
...
use "$directorio/DB/Master_with_duplicates.dta", clear
*Trimming
...
qui putexcel B2=matrix(r(matrix)) using "$directorio\Tables\SS_2.xlsx", sheet("SS_survey") modify
...
forvalues t = 1/5 {
	...
	qui putexcel `Col'16=matrix(`obs') using "$directorio\Tables\SS_2.xlsx", sheet("SS_survey") modify
	}
...
foreach var of varlist genero edad  val_pren pres_antes pr_recup masqueprepa response {
	...
	test 1.t_prod==2.t_prod==3.t_prod==4.t_prod==5.t_prod
	local p_val = `r(p)'
	qui putexcel Q`i'=matrix(`p_val') using "$directorio\Tables\SS_2.xlsx", sheet("SS_survey") modify
	local i = `i'+2
	}
...
qui putexcel B2=matrix(r(matrix)) using "$directorio\Tables\SS_2.xlsx", sheet("SS_survey_exit") modify
...
foreach var of varlist contrataria satisfied_esquema_pago sit_econ_mejor optaria_mensual ///
	...
	test 1.t_prod==2.t_prod==3.t_prod==4.t_prod==5.t_prod
	local p_val = `r(p)'
	qui putexcel Q`i'=matrix(`p_val') using "$directorio\Tables\SS_2.xlsx", sheet("SS_survey_exit") modify
	local i = `i'+2
	}	
