...
* Author:	Isaac M
* Machine:	Isaac M 											
* Date of creation:	November. 4, 2021 (from iv_reg_detailed.do)
* Last date of modification: March. 2, 2021 
* Modifications: -Added conditional when experiencing previous contracts
	- Refine murky cases definition
* Files used:     
		- 
* Files created:  
...
preserve
collapse (max) pf_client = pago_fijo (mean) def (mean) plazoprestamo, by(idcliente date_opening date_closing)
...
bysort idcliente : gen within_three_months = (date_opening-12 <= date_closing[_n-1])
bysort idcliente: gen num_pawns = _N
bysort idcliente: gen f_idcliente = (_n==1)
gen flag_aux = (num_pawns>=2 & within_three_months==1)
...
by idcliente: gen previous_credit_closed_n = 1 /*All credits*/
...
*Dummy variable identifying opening of credit after having EXPERIENCED previous loan for contract term + `wkc' weeks

foreach wkc in 0 1 2 3 4 {
	by idcliente: gen previous_credit_exp_`wkc' = (date_opening>=date_opening[_n-1]+floor(plazoprestamo/7)+`wkc') if date_opening>date_opening[_n-1]
	by idcliente: replace previous_credit_exp_`wkc' = . if _n==1
	}
...
eststo clear
	...
	*###############################################################################				
		*Previous loan closed	
		cap drop insample
		gen insample = 1 if pf_suc==1 & previous_credit_closed_`wkc'==1 & !missing(demand_past_immn) & !missing(active_pastn)
		sort idcliente insample date_ope
		cap drop num_learning
		*Number of 'decision' epochs
		by idcliente insample : gen num_learning = _n if insample==1	
				  
			*Keep observations with the first time of "switch" (just two pawns)
		eststo : ivreghdfe pago_fijo (demand_past_immn=active_pastn) if pf_suc==1 & previous_credit_closed_`wkc'==1 & num_learning==1, absorb(date_opening)	 robust
		su pago_fijo if e(sample) 
		estadd scalar DepVarMean = `r(mean)'
		
			*IV -OLS
		eststo: ivreghdfe pago_fijo i.num_learning (demand_past_immn=active_pastn) if pf_suc==1 & previous_credit_closed_`wkc'==1, absorb(date_opening) cluster(idcliente)
		su pago_fijo if e(sample) 
		estadd scalar DepVarMean = `r(mean)'

			*Reduced form 
		eststo: reghdfe pago_fijo  active_pastn  i.num_learning if pf_suc==1 & previous_credit_closed_`wkc'==1, absorb(date_opening) vce(cluster idcliente)
		su pago_fijo if e(sample) 
		estadd scalar DepVarMean = `r(mean)'
			*Reduced form (conditional on not choosing)
		eststo: reghdfe pago_fijo  active_pastn  i.num_learning if pf_suc==1 & previous_credit_closed_`wkc'==1 & demand_past_immn==0, absorb(date_opening) vce(cluster idcliente)
		su pago_fijo if e(sample) 
		estadd scalar DepVarMean = `r(mean)'
	...
	*-------------------------------------------------------------------------------
			...
			*IV -FE
		eststo: ivreghdfe pago_fijo (demand_past_immn=active_pastn) if pf_suc==1 & previous_credit_closed_`wkc'==1, absorb(idcliente date_opening) cluster(idcliente)
		su pago_fijo if e(sample) 
		estadd scalar DepVarMean = `r(mean)'

			*Reduced form (FE)
		eststo: reghdfe pago_fijo  active_pastn if pf_suc==1 & previous_credit_closed_`wkc'==1, absorb(idcliente date_opening) vce(cluster idcliente)
		su pago_fijo if e(sample) 
		estadd scalar DepVarMean = `r(mean)'
			*Reduced form (conditional on not choosing)
		eststo: reghdfe pago_fijo  active_pastn if pf_suc==1 & previous_credit_closed_`wkc'==1 & demand_past_immn==0, absorb(idcliente date_opening) vce(cluster idcliente)
		su pago_fijo if e(sample) 
		estadd scalar DepVarMean = `r(mean)'
		
		
*###############################################################################				
		*Experience previous loan
		cap drop insample
		gen insample = 1 if pf_suc==1 & previous_credit_exp_`wkc'==1 & !missing(demand_past_immn) & !missing(active_pastn)
		sort idcliente insample date_ope
		cap drop num_learning
		*Number of 'decision' epochs
		by idcliente insample : gen num_learning = _n if insample==1	
			...
			*Keep observations with the first time of "switch" (just two pawns)
		eststo : ivreghdfe pago_fijo (demand_past_immn=active_pastn) if pf_suc==1 & previous_credit_exp_`wkc'==1 & num_learning==1, absorb(date_opening)	 robust
		su pago_fijo if e(sample) 
		estadd scalar DepVarMean = `r(mean)'
		
			*IV -OLS
		eststo: ivreghdfe pago_fijo i.num_learning (demand_past_immn=active_pastn) if pf_suc==1 & previous_credit_exp_`wkc'==1, absorb(date_opening) cluster(idcliente)
		su pago_fijo if e(sample) 
		estadd scalar DepVarMean = `r(mean)'

			*Reduced form 
		eststo: reghdfe pago_fijo  active_pastn  i.num_learning if pf_suc==1 & previous_credit_exp_`wkc'==1, absorb(date_opening) vce(cluster idcliente)
		su pago_fijo if e(sample) 
		estadd scalar DepVarMean = `r(mean)'
			*Reduced form (conditional on not choosing)
		eststo: reghdfe pago_fijo  active_pastn  i.num_learning if pf_suc==1 & previous_credit_exp_`wkc'==1 & demand_past_immn==0, absorb(date_opening) vce(cluster idcliente)
		su pago_fijo if e(sample) 
		estadd scalar DepVarMean = `r(mean)'
	...
	*-------------------------------------------------------------------------------
			...
			*IV -FE
		eststo: ivreghdfe pago_fijo (demand_past_immn=active_pastn) if pf_suc==1 & previous_credit_exp_`wkc'==1, absorb(idcliente date_opening) cluster(idcliente)
		su pago_fijo if e(sample) 
		estadd scalar DepVarMean = `r(mean)'

			*Reduced form (FE)
		eststo: reghdfe pago_fijo  active_pastn if pf_suc==1 & previous_credit_exp_`wkc'==1, absorb(idcliente date_opening) vce(cluster idcliente)
		su pago_fijo if e(sample) 
		estadd scalar DepVarMean = `r(mean)'
			*Reduced form (conditional on not choosing)
		eststo: reghdfe pago_fijo  active_pastn if pf_suc==1 & previous_credit_exp_`wkc'==1 & demand_past_immn==0, absorb(idcliente date_opening) vce(cluster idcliente)
		su pago_fijo if e(sample) 
		estadd scalar DepVarMean = `r(mean)'
	...
	*Save results	
	esttab using "$directorio/Tables/reg_results/learning_not_doing.csv", se r2 ${star} b(a2) ///
			scalars("DepVarMean DepVarMean") keep(demand_past_immn active_pastn) replace 
