/*
Reincidence treatment effect - 30/60 days before
*/
...
collapse reincidence_bef* prestamo $C1  pro_2 ///
...
matrix results = J(2, 5, .) // empty matrix for results
	//  5 cols are: (1) Treatment arm, (2) beta, (3) std error, (4) df, (5) pvalue	
	
local row = 1	

foreach var of varlist reincidence_bef30 reincidence_bef60 {
	reg `var' pro_2 ${C1} , r 
	local df = e(df_r)	
...
graph twoway 
			...
			(scatter beta k if p>=0.05 , `estimate_options_90') 
			...
			legend(off)
			xlabel(1 "reincidence (<30)" 
			2 "reincidence (<60)", angle(horizontal) labsize(small))
			;

#delimit cr
graph export "$directorio\Figuras\re_te_earlydays.pdf", replace
