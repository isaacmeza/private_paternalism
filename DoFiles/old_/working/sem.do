...
gen default = def_c if choose_commitment==0
 /*
fmm 2 if t_prod==4,  lcprob(genero log_prestamo pr_recup pres_antes ///
	plan_gasto pb   choose_commitment   faltas) emopts(iter(500))  difficult technique(nr dfp bfgs) startvalues(randomid)  : mlogit def_c 

fmm 2 if t_prod==4,   emopts(iter(500))  difficult technique(nr dfp bfgs) startvalues(randomid)  : mlogit def_c log_prestamo


gsem (def_c <- log_prestamo) if t_prod==4, logit lclass(C 2) startvalues(randomid, draws(5) seed(15))
...
global C = `r(N)'
br log_prestamo def_c

**(2) Split the sample**
generate double _p = runiform() 
local prop 0.25
gen _types =  irecode(runiform(),`prop',1)
replace _types = _types + 1
