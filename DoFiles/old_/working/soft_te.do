...
(des_c_p, keep(3.t_producto) rename(3.t_producto = "Recovery") color(navy) cismooth(color(navy) n(10)) offset(0.04)) /// 
...
(des_c_p, keep(5.t_producto) rename(5.t_producto = "Recovery") color(maroon) cismooth(color(maroon) n(10))  offset(-0.04)) ///
...
, nooffset legend(order(11 "Forced-soft" 66 "Choice-soft") pos(6) rows(1)) xline(0, lcolor(gs10))  graphregion(color(white)) xtitle("T. Effects (std deviations)")
