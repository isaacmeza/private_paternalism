...
*Scatter plot
twoway (scatter tau_sim_narrow tau_sim, msymbol(smcircle_hollow) text(0.4 -0.4 "R-squared : `r2'")) (lfitci tau_sim_narrow tau_sim, lcolor(navy)) (line tau_sim tau_sim, sort lcolor(black) lpattern(dash)),  name(sct,replace)  legend(off) xsc(off) ysc(off)  ///
xti("") yti("") xlab(-0.5(.25)0.5,nolab) ylab(-0.5(.25)0.5,nolab)

*Marginals
kdensity tau_sim_narrow, k(gauss) gen(x0 d0)
line x0 d0, xsc(rev off) ysc(alt) xlab(,nolab) ylab(-0.5(.25)0.5) xtick(,notick) name(hist0, replace) fxsize(40) ytitle("Narrow forest")
kdensity tau_sim,  k(gauss) gen(x1 d1)
line d1 x1, xsc(alt) ysc(rev off) ylab(,nolab) xlab(-0.5(.25)0.5) ytick(,notick) name(hist1, replace) fysize(35) xtitle("Wide forest")
