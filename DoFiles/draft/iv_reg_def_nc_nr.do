...
* Author:	Isaac M
* Machine:	Isaac M 											
* Date of creation:	October. 19, 2021
...
*				REGRESSIONS				   *
********************************************
duplicates drop idcliente idsucursal date_opening pf_suc def demand_past_immn active_pastn, force
gen refrendo = (nref==0)
...
*FS
reghdfe pago_fijo pf_suc, absorb(idcliente date_opening)
cap drop esample
...
foreach wk in n /*2 4 6 8 10 12 15*/ {
	...
	*FE
		*FS
	reghdfe demand_past_imm`wk' active_past`wk'    ///
	...
	gen residual = demand_past_imm`wk' - pr 
		...
		*IV
	eststo: reghdfe def demand_past_imm`wk' residual  if esample, absorb(idcliente date_opening) vce(cluster idcliente)
	su def if e(sample) 
	estadd scalar DepVarMean = `r(mean)'

		*IV (conditional on not referendums)
	eststo: reghdfe def pr if esample & nref, absorb(idcliente date_opening) vce(cluster idcliente)
	su def if e(sample) 
	estadd scalar DepVarMean = `r(mean)'
	
		*IV (conditional on not choosing)
	eststo: reghdfe def pr  if esample & demand_past_imm`wk'==0, absorb(idcliente date_opening) vce(cluster idcliente)
	su def if e(sample) 
	estadd scalar DepVarMean = `r(mean)'

		*IV (conditional on not choosing & not refrendum)
	eststo: reghdfe def pr  if esample & demand_past_imm`wk'==0 & nref, absorb(idcliente date_opening) vce(cluster idcliente)
	su def if e(sample) 
	estadd scalar DepVarMean = `r(mean)'
	
*------------------------------------------

	*Interaction endogeneous var
	cap drop nref_dem
	gen nref_dem = nref*demand_past_imm`wk'
	*Interaction instrument var
	cap drop nref_active
	gen nref_active = nref*active_past`wk'

		*FS 
	reghdfe demand_past_imm`wk' active_past`wk'  nref_active nref  if pf_suc==1 & previous_credit_closed_`wkc'==1, absorb(idcliente date_opening )
	cap drop pr1
	predict pr1 if e(sample)
	reghdfe nref_dem active_past`wk' nref_active nref  if pf_suc==1 & previous_credit_closed_0==1, absorb(idcliente date_opening)	
	cap drop pr2	
	predict pr2 if e(sample)
	
		*IV
	eststo: reghdfe def pr1 pr2 nref if pf_suc==1 & previous_credit_closed_0==1 , absorb(idcliente date_opening)  vce(cluster idcliente)	
	su def if e(sample) 
	estadd scalar DepVarMean = `r(mean)'
...
*Save results	
esttab using "$directorio/Tables/reg_results/iv_reg_def_nc_nr.csv", se r2 ${star} b(a2) ///
