...
foreach wk in n /*2 4 6 8 10 12 15*/ {
	...
	eststo: reghdfe pago_fijo  pr  if esample & demand_past_imm`wk'==0, absorb(date_opening) vce(cluster idcliente)
	su pago_fijo if e(sample) 
	estadd scalar DepVarMean = `r(mean)'

		*Reduced form 
	eststo: reghdfe pago_fijo  active_past`wk'  if pf_suc==1 & previous_credit_closed_`wkc'==1, absorb(date_opening) vce(cluster idcliente)
	su pago_fijo if e(sample) 
	estadd scalar DepVarMean = `r(mean)'
	
		*Reduced form (conditional on not choosing)
	eststo: reghdfe pago_fijo  active_past`wk'  if pf_suc==1 & previous_credit_closed_`wkc'==1 & demand_past_imm`wk'==0, absorb(date_opening) vce(cluster idcliente)
	su pago_fijo if e(sample) 
	estadd scalar DepVarMean = `r(mean)'
...
*-------------------------------------------------------------------------------
	...
	eststo: reghdfe pago_fijo pr    if esample & demand_past_imm`wk'==0, absorb(idcliente date_opening) vce(cluster idcliente)
	su pago_fijo if e(sample) 
	estadd scalar DepVarMean = `r(mean)'

		*Reduced form 
	eststo: reghdfe pago_fijo active_past`wk' if pf_suc==1 & previous_credit_closed_`wkc'==1 , absorb(idcliente date_opening) vce(cluster idcliente)
	su pago_fijo if e(sample) 
	estadd scalar DepVarMean = `r(mean)'
	
		*Reduced form (conditional on not choosing)
	eststo: reghdfe pago_fijo active_past`wk' if pf_suc==1 & previous_credit_closed_`wkc'==1 & demand_past_imm`wk'==0, absorb(idcliente date_opening) vce(cluster idcliente)
	su pago_fijo if e(sample) 
	estadd scalar DepVarMean = `r(mean)'
...
*Save results	
esttab using "$directorio/Tables/reg_results/iv_reg_demand_pf_notchoosing.csv", se r2 ${star} b(a2) ///
