...
*-------------------------------------------------------------------------------
foreach var of varlist tau_apr tau_eff tau_des tau_def {
...
local row = 1
foreach var of varlist val_pren_std faltas pb plan_gasto_bin pres_antes edad genero masqueprepa {
...
mat rownames choice =  "Subjective value (std)" ///
...
coefplot (matrix(choice[,2]), ci((choice[,5] choice[,6]))  ciopts(lcolor(gs4))), ///
	headings("Subjective value (std)" = "{bf:Loan characteristics}" "Income index" = "{bf:Income}" "Present bias" = "{bf:Self Control}" "Pawn before" = "{bf:Experience}" "Age" = "{bf:Other}",labsize(medium)) legend(off) offset(0) xline(0)  graphregion(color(white)) 
