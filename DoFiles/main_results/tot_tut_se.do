...
eststo : ivregress 2sls apr z1 $C0 (x2 = z2), vce(cluster suc_x_dia)
test z1 = x2
estadd scalar ate_tot = `r(p)'
	*Diagnostics
ivreg2 apr z1 (x2 = z2), cluster(suc_x_dia) first endog(x2) orthog(z2)
	*Reduced form 
qui su choose_commitment 
local p_rate = `r(mean)'	
...
estadd scalar tot_se = `tot_se'
	*GMM
eststo : gmm (apr - {tot: x2 z1} - {b0}), instruments(z1 z2) vce(cluster suc_x_dia) twostep	
qui su apr if e(sample) & t_prod==1
local mn = `r(mean)'
test z1 = x2
estadd scalar ate_tot = `r(p)'

eststo : gmm (apr - {tot: x2 z1 $C0} - {b0}), instruments(z1 z2 $C0) vce(cluster suc_x_dia) twostep 
qui su apr if e(sample) & t_prod==1
local mn = `r(mean)'
estadd scalar ContrMean = `mn'
test z1 = x2
...
eststo : ivregress 2sls apr $C0 (choice_nonsq =  forced_fee_vs_choice) , vce(cluster suc_x_dia)
	*LATE + ATE
eststo : ivregress 2sls apr z0 (x1 = z2), vce(cluster suc_x_dia)
test z0 = x1
estadd scalar ate_tut = `r(p)'
eststo : ivregress 2sls apr z0 $C0 (x1 = z2), vce(cluster suc_x_dia)
test z0 = x1
estadd scalar ate_tut = `r(p)'
	*Diagnostics
ivreg2 apr z0 (x1 = z2), cluster(suc_x_dia) first endog(x1) orthog(z2)
	*Reduced form 
qui su choose_commitment 
local p_rate = `r(mean)'	
eststo : reg apr i.t_prod ,  vce(cluster suc_x_dia)	 
...
eststo : gmm (apr - {tut: x1 z0} - {b0}), instruments(z0 z2) vce(cluster suc_x_dia) twostep  from(initval) winitial(identity)
qui su apr if e(sample) & t_prod==1
local mn = `r(mean)'
estadd scalar ContrMean = `mn'
test z0 = x1
...
eststo : gmm (apr - {tut: x1 z0 $C0} - {b0}), instruments(z0 z2 $C0) vce(cluster suc_x_dia) twostep  winitial(identity)
qui su apr if e(sample) & t_prod==1
local mn = `r(mean)'
estadd scalar ContrMean = `mn'
test z0 = x1
...
eststo : gmm (apr - {tot: x2 z1} - {b1}) (apr - {tut: x1 z0} - {b0}), instruments(1: z1 z2) instruments(2: z0 z2) vce(cluster suc_x_dia) twostep  from(initval) winitial(identity)
qui su apr if e(sample) & t_prod==1
local mn = `r(mean)'
estadd scalar ContrMean = `mn'
test z0 = x1
...
eststo : gmm (apr - {tot: x2 z1 $C0} - {b1}) (apr - {tut: x1 z0 $C0} - {b0}), instruments(1: z1 z2 $C0) instruments(2: z0 z2 $C0) vce(cluster suc_x_dia) twostep  winitial(identity)
qui su apr if e(sample) & t_prod==1
local mn = `r(mean)'
estadd scalar ContrMean = `mn'
test z0 = x1
