...
data_in <- read_csv('./_aux/def_te_heterogeneity.csv') %>%
...
data_test <- data_in %>% 
  select(-c(def_c, fee_arms, prenda, insample))
...
# PREPARE VARIABLES
X <- select(data_train,-c(def_c, fee_arms, prenda, insample))
Y <- select(data_train,def_c)
W <- as.numeric(data_train$fee_arms == 1)

X_full <- select(data_in,-c(def_c, fee_arms, prenda, insample))
Y_full <- select(data_in,def_c)
W_full <- as.numeric(data_in$fee_arms == 1)

X_fee <- select(data_train_fee,-c(def_c, fee_arms, prenda, insample))
Y_fee <- as.numeric(data_train_fee$def_c)
X_nofee <- select(data_train_nofee,-c(def_c, fee_arms, prenda, insample))
Y_nofee <- as.numeric(data_train_nofee$def_c)
...
filename <- paste("_aux/def_te_grf",".csv", sep="") 
