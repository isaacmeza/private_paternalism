...
if "`depvar'" == "des_c" & "`treat'" == "pro_2"{
	mat rownames family = "Common_asks" 
	mat rownames income = "Water" "Medicine" "Electricity" "Gas" ///
	"Food" "Phone" "Rent" 
	mat rownames selfc = "Present_bias" ///
	"Makes_budget" "Tempt"
	mat rownames experience = "Pawn_before"  "Rosca" 
	mat rownames other = "Stressed" "Age" "Loan_amt" "Prob_recovery" "Female" ///
	 "Low_time" "More_high_school"  "Low_cost"	
...
if "`depvar'" == "des_c" & "`treat'" == "pro_3"{
	...
	mat rownames selfc = "Tempt" "Makes_budget" "Present_bias"   
	mat rownames experience = "Pawn_before"  ///
	"Rosca" 
	mat rownames other = "Stressed" "Gender" "Age" "Loan_amt"  ///
	 "Prob_recovery" "Low_time" "More_high_school" "Low_cost"	
...
coefplot (matrix(family[,2]), ci((family[,5] family[,6])) `options1' ciopts(lcolor(gs11))) /// 
	...
	(matrix(experience[,2]), ci((experience[,5] experience[,6])) `options4' ciopts(lcolor(gs6))) ///
	(matrix(other[,2]), ci((other[,5] other[,6])) `options5' ciopts(lcolor(gs4))), ///
	headings("Common_asks" = "{bf:Family}" "Food" = "{bf:Income}" ///
	"Tempt" = "{bf:Self Control}" "Pawn_before" = "{bf:Experience}" "Stressed" = "{bf:Other}",labsize(vsmall)) ///
	legend(off) offset(0) xline(0) `graphregion' `plotregion' ylabel(,labsize( small ))	
...
if "`depvar'" == "des_c" & "`treat'" == "pro_4"{
	...
	mat rownames experience = "Pawn_before" ///
	"Rosca" 
	mat rownames other = "Stressed" "Female" "Low_time" ///
	"Loan_amt" "Age"  "Prob_recovery" "Low_cost"  ///
	 "More_high_school" 
...
coefplot (matrix(family[,2]), ci((family[,5] family[,6])) `options1' ciopts(lcolor(gs11))) /// 
	...
	(matrix(experience[,2]), ci((experience[,5] experience[,6])) `options4' ciopts(lcolor(gs6))) ///
	(matrix(other[,2]), ci((other[,5] other[,6])) `options5' ciopts(lcolor(gs4))), ///
	headings("Common_asks" = "{bf:Family}" "Phone" = "{bf:Income}" ///
	"Tempt" = "{bf:Self Control}" "Pawn_before" = "{bf:Experience}" "Stressed" = "{bf:Other}",labsize(vsmall)) ///
	legend(off) offset(0) xline(0) `graphregion' `plotregion' ylabel(,labsize( small ))	
...
if "`depvar'" == "des_c" & "`treat'" == "pro_5"{
	...
	"Gas" "Electricity"
	mat rownames selfc = "Tempt" "Present_bias" "Makes_budget"    
	mat rownames experience = "Rosca" "Pawn_before" 
	mat rownames other = "Stressed" "More_high_school" ///
	"Loan_amt" "Prob_recovery" "Age" ///
	"Female" "Low_cost" "Low_time" 	
...
coefplot (matrix(family[,2]), ci((family[,5] family[,6])) `options1' ciopts(lcolor(gs11))) /// 
	...
	(matrix(other[,2]), ci((other[,5] other[,6])) `options5' ciopts(lcolor(gs4)))	, ///
	headings("Common_asks" = "{bf:Family}" "Phone" = "{bf:Income}" ///
	"Tempt" = "{bf:Self Control}" "Rosca" = "{bf:Experience}" "Stressed" = "{bf:Other}",labsize(vsmall)) ///
	legend(off) offset(0) xline(0) `graphregion' `plotregion' ylabel(,labsize( small ))	
