...
foreach depvar in des_c  dias_al_desempenyo  num_p  sum_porcp_c ref_c reincidence {
	...
	forvalues t = 2/5 {
		...
		local vrlist prestamo pr_recup edad  /// *Continuous covariates
	genero pres_antes fam_comun cta_tanda /// *Dummy variables
	renta comida medicina luz gas telefono agua  ///
	masqueprepa estresado_seguido pb hace_presupuesto tentado low_cost low_time
	
		local vrlistnames loan.amt pr.recovery  age /// *Continuous covariates
	gender pawn.before common.asks relay /// *Dummy variables
	...
	*local self_control pb fb hace_presupuesto tentado
	local self_control pb hace_presupuesto tentado
	local experiencia pres_antes cta_tanda 
	local otros prestamo edad genero masqueprepa pr_recup estresado_seguido low_cost low_time
	
	local familianames fam.asks  
	local ingresonames rent food medicine electricity gas phone water  
	local self_controlnames pb makes.budget tempt
	local experiencianames pawn.before relay 
	local otrosnames loan.amt age female more.high.school pr.recovery stressed low.cost low.time
