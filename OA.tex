...
\todo[inline]{For Isaac to write the note.}
\end{landscape}
...
For these loans, we do not know whether the borrower ultimately defaulted our recovered her pawn.
In this appendix, we show that our main results are robust to different ways of addressing censoring.

Our main results, presented above, assumed that all loans that were still outstanding at the end of the sample period were \emph{eventually repaid}.
Because forced commitment causes borrowers to make payments earlier than they otherwise would have, this is a conservative assumption.
To see why, suppose for the sake of argument that commitment affects payment timing, but does not not default.
...
This is why we adopt the reverse convention above.
We now consider some alternative approaches.


\todo[inline]{@Frank: keep updating below after getting responses from @Issac.}


%The approach taken in the paper for censored loans is a conservative one in that it inherently assumes that all of the loans outstanding at the end of the observation window will be repaid, making it so that the acceleration of payment observed in the Forced arm does not translate mechanically into a treatment effect on default.  This appendix delves into it more detail.  
...
\begin{figure}[H]
    ...
    \end{center}
     ...
     %to be largely concentrated relevant survey question  according to her survey responses two binary performs a partition of the Treatment on the Untreated (FC), estimating the share of the effect that arises within each of two binary categories.  The top bars examine present bias (individuals are more impatient over one-month time horizon than they are between three and four months).  The bottom bars examine sure confidence (individual reports 100\% probability of repaying at time of taking loan).  Blue bar is the component arising from those without the trait, orange bar from those with the trait. 
      ...
      \todo[inline]{This figure has changed. Will have minor style modifications. @Issac: what's the purple stuff in the middle?}
\end{figure}
...
%\todo[inline]{Presumably we want to cut some of this text? In the body of the text we don't get into this in such detail.}
%We plot the empirical distribution of financing cost $F_{commitment}(FC)$ and of the status quo contract, $F_{sq}(FC)$. Framed as a choice among cost distributions. 
...
\begin{figure}[H]
   ...
   \begin{subfigure}{0.49\textwidth}
    ...
    \end{subfigure} 
    \end{center}
    \scriptsize This figure plots plots the empirical CDF of experimental outcomes separately for the control / status quo (dashed) and forced commitment contracts (solid). In panel (a), the outcome is APR; in panel (b) the outcome is Financial Cost. In each case, the empirical CDF under forced commitment first-order stochastically dominates the empirical CDF under the status quo. This can be seen by examining the dotted line at the bottom of panels (a) and (b), which shows the difference $(\text{Control} - \text{Commitment})$. 
...
In the first step, we apply the generalized random forest approach of \cite{athey2019generalized} to the training set to estimate two proxy predictors: $\psi(\cdot|\text{Training})$ approximates the untreated potential outcome function, $\mathbbm{E}[Y_{i0}|X_i] = \mathbbm{E}[Y_i|Z_i=0,X_i]$, while  $\tau(\cdot|\text{Training})$, approximates the ATE function
\[
...
Our estimate for $\beta_2$ is \hl{XXXX} with a cluster-robust standard error of \hl{XXXX}.
Thus we easily reject the null hypothesis of homogenous treatment effects.
...
\begin{figure}[H]
    ...
    \caption{Default}
       \centering
      \includegraphics[width=\textwidth]{Figuras/he_dist_def.pdf}
    \end{subfigure}  
     \begin{subfigure}{0.45\textwidth}
    \caption{Recovery}
       \centering
      \includegraphics[width=\textwidth]{Figuras/he_dist_des.pdf}
    \end{subfigure}      
    \end{center}
         \scriptsize
...
\end{figure}
\todo[inline]{@Isaac: reestimate this with the determinants we are using for the choice determinants. Based on that we decide if we cut}
